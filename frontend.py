...
st.markdown(
    ...
    .heading1 {
        ...
        text-shadow: 4px 4px 8px rgba(37, 0, 100, 0.8);
        margin-top: 20px;
        padding: 10px;
        width: 100%;
    }
...
def homepage():
    ...
    st.markdown('<p class="bodytext">StemFlow is a high-level machine learning model for music source separation. '
                'It uses deep neural networks to split musical compositions into individual track stems.</p>', unsafe_allow_html=True)
...
def about_stemflow():
    ...
    st.markdown('<p class="bodytext">StemFlow operates using the Demucs model (created by Meta). '
                'You can find the repository below:</p>', unsafe_allow_html=True)
    st.markdown("[Demucs Repository](https://github.com/facebookresearch/demucs)")

    st.markdown(
        '<p class="bodytext">Demucs uses a U-Net architecture for source separation. '
        'It employs an encoder-decoder system where the encoder extracts features from the input (the mixed audio) '
        'and reduces dimensionality, while the decoder reconstructs individual sources. The key innovation in U-Net '
        'is the skip connections, which allow fine details to be preserved.</p>', 
        unsafe_allow_html=True
    )

    st.markdown(
        '<p class="bodytext">Another feature that Demucs employs is Recurrent Neural Networks (RNNs). '
        'This is useful for stem splitting as RNNs can access past data and make predictions, '
        'which helps efficiently separate instruments in a track.</p>', 
        unsafe_allow_html=True
    )
